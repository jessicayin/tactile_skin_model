...
class ExperienceBuffer(Dataset):
    def __init__(
        ...
        self.storage_dict = {
            ...
            "critic_info": torch.zeros(
                (self.transitions_per_env, self.num_envs, self.critic_info_dim),
                dtype=torch.float32,
                device=self.device,
            ),
            ...
            #     device=self.device,
            # ),
            "rot_axis_buf": torch.zeros(
                (self.transitions_per_env, self.num_envs, 3),
                dtype=torch.float32,
                device=self.device,
            ),
            "rewards": torch.zeros(
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            input_dict["critic_info"],
            # input_dict["point_cloud_info"],
            input_dict["rot_axis_buf"],
        )
