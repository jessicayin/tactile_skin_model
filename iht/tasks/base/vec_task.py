...
class VecTask(Env):
    ...
    def _allocate_buffers(self):
        ...
        #self.num_obs: 16 * 6 for joint angles = 96, +3 for object goal position. final shape should be 35.
        obs_buf_lag_history_dim = 35 #32 joint angles + 3 obj goal pos
        obs_buf_dim = self.num_obs
        # if self.enable_palm_reskin:
        #     obs_buf_lag_history_dim = 83 #35 + (16 sensors * 3 axis forces) = 83
        #     obs_buf_dim = 147 # 96 joint angles + 3 obj goal + (16 sensors * 3 axis force) = 147
        self.obs_buf = torch.zeros(
            (self.num_envs, obs_buf_dim), device=self.device, dtype=torch.float
        )
        self.obs_buf_lag_history = torch.zeros(
            (self.num_envs, 80, obs_buf_lag_history_dim), 
            device=self.device,
            dtype=torch.float,
        )
    ...
    def render(self):
        ...
        # if self.enable_camera_sensors and not self.fast_depth_simulation:
    ...
    def _parse_sim_params(
        ...
        #weird bug but this line is needed for reskin sensors to output force values
